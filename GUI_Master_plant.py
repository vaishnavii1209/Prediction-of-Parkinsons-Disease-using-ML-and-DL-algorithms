...
#
lbl = tk.Label(root, text="Parkison's disease Detection", font=('times', 25,' bold '), height=1, width=70,bg="black",fg="white")
lbl.place(x=0, y=20)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="#696969",fg="black")
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=10, y=150)
...
#    global fn
    ...
    if fn!="":
        ...
        if plant == 0:
            Cd="parkisons disease not"
        elif plant == 1:
            Cd="parkisons disease"
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/Ravina data/parkisons disease/dataset', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
exit = tk.Button(frame_alpr, text="Exit", command=window, width=15, height=1, font=('times', 15, ' bold '),bg="#2F4F4F",fg="white")
